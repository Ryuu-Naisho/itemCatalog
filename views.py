...
def gconnect():
    ...
    if result['user_id'] != gplus_id:
        response = make_response(
            json.dumps(("Token's user ID doesn't match the given user"
                        "ID.")), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if not userExists(user_info['email']):
        createUser(user_info['name'], user_info['email'],
                   user_info['picture'], login_session['access_token'])
...
def gdisconnect():
    ...
    if access_token is None:
        response = make_response(json.dumps("There's nothing to log out off."
                                            ), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def getCategory(category_name):
    ''' Returns the category template and displays items that belong to the
    category.'''
...
def deleteCategory(category_name):
    ...
    return render_template('delete_category.html',
                           category_name=category_name, user=getUser())
...
def createItem(category_name=None):
    ...
    if request.method == 'POST':
        ...
        ''' Check if request came from a category page. i.e category_name
        should be the name of that category. '''
...
def createUser(username, email, picture, password):
    ''' Creates a new user, if oAuth user, password will be the access toeken
